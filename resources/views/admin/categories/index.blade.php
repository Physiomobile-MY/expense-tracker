...
<section class="pm-card mb-4 p-4">
    <form method="POST" action="{{ route('admin.categories.store') }}" class="grid gap-3 lg:grid-cols-[1fr_0.55fr_1fr_1.3fr_0.5fr_auto]">
        @csrf
        ...
        <textarea class="pm-input min-h-12" name="keywords_text" placeholder="Auto-match keywords, comma or line separated"></textarea>
...
<section class="pm-card overflow-hidden">
    <div class="divide-y divide-gray-100">
        @foreach ($categories as $category)
            <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="grid gap-3 px-4 py-4 lg:grid-cols-[1fr_0.55fr_1fr_1.3fr_0.5fr_auto]">
                @csrf
                @method('PUT')
                ...
                <textarea class="pm-input min-h-12" name="keywords_text" aria-label="Keywords for {{ $category->name }}">{{ implode(', ', $category->keywords ?? []) }}</textarea>
                <select class="pm-input" name="status">
